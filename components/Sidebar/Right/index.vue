<template>
  <div class="flex flex-col">
    <SidebarRightPreviewCard title="What's Happening?">
      <SidebarRightPreviewCardItem v-for="whatsHappening in whatsHappeningItems">
        <div>
          <h2 class="font-bold text-gray-800 text-md dark:text-white">{{ whatsHappening.title }}</h2>
        <p class="text-xs text-gray-400">
          {{ whatsHappening.count }}
        </p>
        </div>
       
      </SidebarRightPreviewCardItem>
    </SidebarRightPreviewCard>
  </div>
  <div class="flex flex-col">
    <SidebarRightPreviewCard title="Who to follow?" >
        <SidebarRightPreviewCardItem v-for="whoToFollow in whoToFollowItems">
            <div class="flex flex-row justify-between p-2 items-center">
              <div class="flex flex-row">
                  <img class="w-10 h-10 rounded-full" :src="whoToFollow.image" :alt="whoToFollow.name"/>
                  <div class="flex flex-col ml-2">
                    <h1 class="text-sm font-bold text-gray-900 dark:text-white">{{ whoToFollow.name }}</h1>
                    <p class="text-xs text-gray-400">{{ whoToFollow.handle }}</p>
                  </div>
              </div>
              <div class="flex flex-row h-full ">
                <button class=" px-4 py-2 font-bold text-xs rounded-full text-white dark:text-black bg-black dark:bg-white hover:bg-gray-700 dark:hover:bg-gray-100  ">Follow</button>
              </div>
            </div>
        </SidebarRightPreviewCardItem>
    </SidebarRightPreviewCard>
  </div>
</template>

<script lang="ts" setup>

  const whatsHappeningItems = ref([
    {title: 'SpaceX',
      count: '18.8k Tweets'
    },
    {
      title: '#Coding is Fun',
      count: '8.8k tweets'
    },
    {
  title: '#Art is Fun',
      count: '1.8k tweets'
    }
  ])

  const whoToFollowItems = ref([
    {name: 'Joe Biden',
      handle: '@JoeBiden',
      image: 'https://picsum.photos/200/200'
    },
    {name: 'XQC',
      handle: '@XQC',
      image: 'https://picsum.photos/200/200'
    },
    {name: 'Shroud',
      handle: '@Shroud',
      image: 'https://picsum.photos/200/200'
    },
  ])
 
  
</script>

<style>

</style>